class MongoDB {
    ...
    async aggregateAvgPieData() {
        let sum = 0, powerMeterPower = 0, upsPower_A = 0, Avg_temp = 0;
        var timeStart = new Date(new Date(new Date().setHours(new Date().getHours() - 1,0,0)) );
        var timeEnd = new Date(new Date(new Date().setHours(new Date().getHours() ,0,0)));
        var HourStart = timeStart.toLocaleString('zh-hant', { timeZone: 'Asia/Taipei' })
        var HourEnd = timeEnd.toLocaleString('zh-hant', { timeZone: 'Asia/Taipei' })
        console.log(HourStart)
        console.log(HourEnd)
        powerMeterPower = await this.avgPower('CO2',HourStart, HourEnd);
        upsPower_A = await this.avgPower('humid',HourStart, HourEnd);
        Avg_temp = await this.avgPower('tempareture',HourStart, HourEnd);
    ...
    async yesterdayAvgPowerRobot() {
        ...
        console.log(`start time:${new Date(HourStart).toLocaleString({ timeZone: "Asia/Taipei" })} , end time:${new Date(HourEnd).toLocaleString({ timeZone: "Asia/Taipei" })}`);
        let yesterdayPowerMeterPower = await this.yesterdayAvgPower('powerMeterPower', HourStart, HourEnd);
        let yesterdayUpsPower_A = await this.yesterdayAvgPower('upsPower_A', HourStart, HourEnd);
        let yesterdayUpsPower_B = await this.yesterdayAvgPower('upsPower_B', HourStart, HourEnd);
        let yesterdayWaterTankPower = await this.yesterdayAvgPower('waterTankPower', HourStart, HourEnd);
